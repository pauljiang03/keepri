...
GSAP3.15.0 is the single engine, using ScrollTrigger for main-page effects and ScrollToPlugin for the final handoff. Intro words exit their masks over340ms and enter over780ms with perspective and a short stagger. The chapter line takes900ms. Contour sculptures morph over1.1–1.3s; the one-way handoff takes1s. The owner explicitly requested smoother, more expressive marketing effects. Center-bottom text rotation and the longer progress-line duration are intentional motion-audit exceptions. One wheel burst or touch swipe advances once, including momentum that continues beyond the animation. Native page scrolling resumes at the main site. Completing or skipping the intro removes it from layout; it cannot be revisited by scrolling up. Reduced motion uses instant changes and handoff. Without JavaScript, native sections remain readable. Cleanup removes listeners and effects.

## Assets

The original brand icon is joined by original code-generated contour artwork in the introduction, hero and experience narrative. The prior game captures are removed from public assets. Each introduction screen retains its single statement, now with animated abstract artwork; there is no product screenshot, game simulation, stock photo, or game-specific decorative ribbon.

Pointer parallax is enabled only for fine pointers without reduced motion. The main site uses native scrolling with a sticky evolving sculpture, masked hero lines, model reveals, page progress, and footer movement. Decorative SVGs are hidden from assistive technology.
