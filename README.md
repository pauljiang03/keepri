...
The publication has exactly three fixed pages: Main, Philosophy and Industry. Main has Overview, Research, Funding and Vision tabs. Philosophy has six selectable principles. Industry has Overview, Environments, Learning data, Evaluation and Process tabs, plus its footer. One topic is visible at a time; the full copy remains in the page. No chapter or panel scrolls. Wheel and touch gestures change chapters only, using the existing fixed 520ms transition. The process panel contains a slow horizontal learning cycle, paused by the footer motion control, hover or reduced-motion preference. Tabs use the installed accessible Base UI primitive through the existing component wrapper. Legacy chapter links remain supported.

The main headline is vertically centered beside its compact, bordered workflow on desktop. Research, funding and vision occupy fixed topic panels. Narrow layouts stack the overview headline and workflow within the screen. Arrows occupy separate layout rows. Every page shares the intro’s forest-green background, cream text and gold accents. Swipe up to go forward and down to go back. Words leave in a short stagger, then the new page’s words enter from the opposite side; backward navigation reverses the movement. Each page change takes 520ms. Diagrams and borders fade with the content. There are no page flips, curls or clipping effects. Text is never individually scaled, stretched or spun. Temporary word wrappers are removed after each transition, restoring the original text nodes and their spacing.
