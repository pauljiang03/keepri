...
Each scroll gesture or touch swipe advances one full-screen statement. Whole-sentence blends and animated decision paths connect the intro to independent reasoning. The hero traces question, attempt, feedback and understanding. A scroll- and button-controlled illustration shows exploration, feedback, revision and returning to a new challenge. Chapter links and a visible skip control remain available. After entry, the intro is removed, so scrolling upward stops at the main site. Research deliverables use native expandable disclosures. Everything remains readable without JavaScript; reduced-motion users get instant chapter changes. Without JavaScript, the intro falls back to three readable native sections.
