...
The intro appears on every load or reload, including URLs with section hashes. It starts paused on the KeepRI mark with a Swipe to spin cue. The first downward wheel gesture, upward swipe, cue click or entry key starts the spin and spelling. The completed phrase holds with a Peel to enter cue; a second gesture starts the 1.65-second peel. Continued touch movement, trackpad momentum, held keys and inputs during the animation cannot skip the second step. Skip intro and Escape dismiss immediately. The cover stays hidden after completion, with no scroll region to return to. Section hashes resolve after entry.
